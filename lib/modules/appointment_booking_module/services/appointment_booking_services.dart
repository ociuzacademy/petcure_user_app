...
class AppointmentBookingServices {
  ...
  static Future<NextVaccineModel> getPetNextVaccine({
    required int petId,
  }) async {
    try {
      final Map<String, dynamic> params = {'pet_id': petId.toString()};

      final url = Uri.parse(
        AppUrls.nextVaccineUrl,
      ).replace(queryParameters: params);
      ...
      if (resp.statusCode == 200) {
        final dynamic decoded = jsonDecode(resp.body);
        final response = NextVaccineModel.fromJson(decoded);
